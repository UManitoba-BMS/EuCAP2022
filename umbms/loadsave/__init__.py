...
def load_birrs_txt(txt_path, return_td=False):
    ...
    return_td : bool
        If True, returns the data in the time-domain, converted by the IDFT

    Returns
    -------
    data : array_like
        2D arr containing the measured radar signal for each antenna position
        used in the scan (in freq-domain by default, in time-domain if
        return_td)
    """
    ...
    if return_td:
        ...
        # Convert the data from the frequency domain to the time domain
        # with the IDFT
        data = np.fft.ifft(fd_data, axis=0)
